...
export default function CheckoutPage({ onBack, onNavigate }) {
  ...
  // Quick auto-fill helper for Stripe testing
  const handleAutoFillTestCard = () => {
    ...
    showToast("Stripe Test Card autofilled (4242 4242...)");
  };
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(result.error || "Payment failed. Please verify your card details.");
      }
    } catch (err) {
      setIsProcessing(false);
  ...
  const handleCopyTransaction = () => {
    if (lastReceipt?.transactionId) {
      navigator.clipboard?.writeText(lastReceipt.transactionId).catch(() => {});
      setCopiedTxn(true);
    ...
    <div className="relative w-full h-full overflow-hidden bg-[#fafbfe] sm:rounded-[44px] flex flex-col justify-between">
      ...
      <div className="w-full pt-3 sm:pt-4 px-6 z-20 shrink-0 bg-[#fafbfe]/95 backdrop-blur-md">
        ...
        <div className="relative flex items-center justify-center py-2">
          <button
            onClick={onBack}
            className="absolute left-0 -ml-2 w-9 h-9 rounded-full flex items-center justify-center text-[#111936] hover:bg-slate-100 active:scale-95 transition-all cursor-pointer"
            title="Go back"
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain touch-pan-y px-5 sm:px-6 space-y-4 pt-1 pb-10 scrollbar-none">
        ...
        <div className="space-y-2">
          ...
          <div className="w-full p-4 rounded-2xl bg-white border border-[#e4e8f7] shadow-[0_2px_8px_rgba(50,70,140,0.03)] space-y-2.5 text-xs">
            <div className="flex justify-between items-center text-[#717ea1] font-medium text-[13px]">
              <span>Map price</span>
              <span className="text-[#0f1738] font-semibold">$12.00</span>
            </div>
            <div className="flex justify-between items-center text-[#717ea1] font-medium text-[13px]">
              <span>Processing fee</span>
              <span className="text-[#0f1738] font-semibold">$0.00</span>
            </div>
            <div className="border-t border-[#edf1f8] pt-2.5 flex justify-between items-center text-[15px] font-black text-[#0f1738]">
              <span>Total</span>
              <span className="text-[#544ee5]">$12.00</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between pl-0.5">
            <h3 className="text-[14.5px] font-bold text-[#0f1738] tracking-tight">
              Payment Method
            </h3>
            <button
              onClick={handleAutoFillTestCard}
              className="text-[11px] font-bold text-[#544ee5] bg-indigo-50 hover:bg-indigo-100 px-2 py-1 rounded-md transition cursor-pointer"
            >
              ⚡ Fill Test Card
            </button>
          </div>
          ...
          <div className="space-y-2.5">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9.5px] font-mono font-bold text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded">
                      Stripe Gateway
                    </span>
                  </div>
              ...
              {/* Interactive Card Input Form with Real-Time Validation */}
              {selectedMethod === 'card' && (
                <div 
                  onClick={(e) => e.stopPropagation()} 
                  className="mt-3.5 pt-3.5 border-t border-[#e8ecf8] space-y-3 animate-in fade-in duration-200"
                >
                  {/* Cardholder Name */}
                  <div>
                    ...
                    <input
                      ...
                      className="w-full h-10 px-3 rounded-xl bg-white border border-[#dbe1f5] text-[13.5px] font-medium text-[#111936] focus:border-[#544ee5] focus:ring-2 focus:ring-[#544ee5]/15 outline-none transition"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        maxLength={19}
                        className="w-full h-10 pl-3 pr-10 rounded-xl bg-white border border-[#dbe1f5] text-[13.5px] font-mono font-medium text-[#111936] focus:border-[#544ee5] focus:ring-2 focus:ring-[#544ee5]/15 outline-none transition tracking-wide"
                      />
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="MM/YY"
                        maxLength={5}
                        className="w-full h-10 px-2.5 rounded-xl bg-white border border-[#dbe1f5] text-[13px] font-mono font-medium text-[#111936] text-center focus:border-[#544ee5] focus:ring-2 focus:ring-[#544ee5]/15 outline-none transition"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-[#717ea1] mb-1">
                  ...
                  <div className="flex items-center justify-between text-[10.5px] text-[#717ea1] pt-1">
                    ...
                    <span className="text-[9.5px] font-mono font-bold text-[#544ee5] bg-indigo-50 px-1.5 py-0.5 rounded">
                      API: pk_test_51...
                    </span>
                  </div>
                </div>
          ...
          >
            {isProcessing ? (
              <span className="flex items-center gap-2">
                ...
                <span>
                  {processingStep === 1 && "Connecting to Stripe..."}
                  {processingStep === 2 && "Authorizing Payment ($12.00)..."}
                  {processingStep === 3 && "Payment Confirmed!"}
                </span>
              </span>
            ) : (
              <>
        ...
        <div className="flex items-center justify-center gap-1.5 text-[12px] text-[#717ea1] font-medium pt-0.5">
          ...
          <span>Secure payment powered by Stripe (pk_test active)</span>
        </div>
        ...
        <div className="w-full p-4 rounded-2xl bg-[#edf0fd] flex items-center gap-3.5">
          ...
          <p className="text-[12px] text-[#556385] leading-snug font-medium">
            After purchase, the complete map with all locations will be immediately available in your library with offline GPS navigation.
          </p>
        </div>
      </div>

      {/* Official Stripe Payment Receipt Modal */}
      {showReceiptModal && (
        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl border border-indigo-50 text-center animate-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-[#717ea1] mb-4">
              Your payment of <strong className="text-[#111936]">$12.00 USD</strong> was processed via Stripe.
            </p>

            {/* Official Receipt Card */}
            <div className="bg-[#f8f9fe] p-3.5 rounded-2xl mb-4 border border-[#e4e8f7] text-left space-y-2 text-xs">
              <div className="flex justify-between items-center text-[#717ea1]">
                <span>Item</span>
                <span className="font-bold text-[#111936]">Paris in 3 Days Map</span>
              </div>
              <div className="flex justify-between items-center text-[#717ea1]">
                <span>Payment Method</span>
                <span className="font-mono font-semibold text-[#111936]">💳 Visa ending in 4242</span>
              </div>
              <div className="flex justify-between items-center text-[#717ea1]">
              ...
              <div className="border-t border-[#e2e7f5] pt-2 flex justify-between items-center text-[#717ea1]">
                <span>Transaction ID</span>
                <button
                  onClick={handleCopyTransaction}
                  className="flex items-center gap-1 font-mono text-[10.5px] text-[#544ee5] font-semibold hover:underline cursor-pointer"
                  title="Copy ID"
                >
                  <span className="truncate max-w-[130px]">
                    {lastReceipt?.transactionId || 'ch_3M42...'}
                  </span>
            ...
            <div className="space-y-2.5">
              <button
                ...
                className="w-full py-3.5 bg-[#544ee5] hover:bg-[#4842db] active:scale-[0.99] text-white font-bold rounded-xl text-sm shadow-md shadow-indigo-200 transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
                <span>Open Purchased Map 🗺️</span>
              ...
              <button
                ...
                className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl text-xs cursor-pointer"
              >
