...
import { Phone, Mail, X, CheckCircle2, ArrowRight } from 'lucide-react';
...
export default function WelcomePage({ onNavigate }) {
  ...
  return (
    <div className="relative w-full h-full min-h-[720px] max-h-[960px] aspect-[9/16] select-none overflow-hidden rounded-[32px] shadow-2xl bg-[#0f1424]">
      ...
      <div className="absolute top-0 left-0 right-0 pt-6 px-6 sm:px-7 flex justify-end z-20">
        <button
          onClick={handleSkip}
      ...
      <div className="absolute left-0 right-0 bottom-0 z-20 flex flex-col items-center px-7 pb-6 pt-2">
        ...
        <div className="w-full flex flex-col gap-[11px]">
          ...
          <button
            ...
            className="w-full h-[52px] sm:h-[55px] bg-white hover:bg-[#fafbff] active:bg-[#f3f4fa] rounded-full flex items-center justify-center gap-3.5 shadow-[0_4px_16px_rgba(40,55,120,0.08)] btn-interactive cursor-pointer group"
          >
          ...
          <div className="w-full max-w-sm bg-white rounded-3xl p-6 shadow-2xl border border-indigo-50 animate-in fade-in zoom-in-95 duration-150">
            ...
            {activeModal === 'phone' && (
              <div className="space-y-4">
                ...
                <button
                  onClick={() => {
                    showToast("Code sent to " + (phoneNumber || "+1 (555) 000-0000"));
                    setActiveModal(null);
                  }}
                  className="w-full py-3 bg-[#554fe2] hover:bg-[#4740d4] text-white font-bold rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-md shadow-indigo-200"
                >
            ...
            {activeModal === 'signin' && (
              <div className="space-y-3.5">
                ...
                <button
                  ...
                  className="w-full py-3 bg-[#554fe2] hover:bg-[#4740d4] text-white font-bold rounded-xl text-sm transition-all shadow-md shadow-indigo-200"
                >
                  Sign In
                </button>
              </div>
            ...
            {(activeModal === 'terms' || activeModal === 'privacy') && (
              <div className="space-y-3 text-xs text-slate-600 max-h-56 overflow-y-auto pr-1">
                ...
                <button
                  onClick={() => setActiveModal(null)}
                  className="w-full mt-2 py-2 bg-slate-100 hover:bg-slate-200 font-semibold rounded-lg text-slate-700"
                >
                  Close
                </button>
              </div>
